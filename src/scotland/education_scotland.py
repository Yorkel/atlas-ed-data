...
# Config
# ----------------------------------------------------------
BASE = "https://education.gov.scot"
START_URL = "https://education.gov.scot/news/"

_DEFAULT_OUTPUT = Path(__file__).resolve().parents[2] / "data" / "training" / "scotland" / "education_scotland.csv"
...
def extract_links(html):
    ...
    links = []
    # Try multiple selectors — page structure is uncertain
...
def extract_next_page(html):
    soup = BeautifulSoup(html, "html.parser")
    next_a = soup.select_one("a.next, a[rel='next'], li.next a, a.pagination__next")
...
def scrape_article(url, since_date=None, until_date=None):
    ...
    # Date parsing — try multiple patterns
    pub_date = None
...
# Main scraper
# ----------------------------------------------------------
def scrape_education_scotland(since_date=None, until_date=None, output_path=None, append=False):
    all_articles = []
    seen = set()
    url = START_URL
    page = 1

    print("Starting Education Scotland scrape...")
    print("  Note: this site may load content via JS — results may be sparse.")
...
if __name__ == "__main__":
    scrape_education_scotland(output_path=_DEFAULT_OUTPUT)
