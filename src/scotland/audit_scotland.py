...
# Config
# ----------------------------------------------------------
BASE = "https://audit.scot"
START_URL = "https://audit.scot/news?keywords%5B486%5D=486"

_DEFAULT_OUTPUT = Path(__file__).resolve().parents[2] / "data" / "training" / "scotland" / "audit_scotland.csv"
...
def extract_links(html):
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"].strip()
        if "/news/" in href and href != "/news/" and href not in [l for l in links]:
...
# Main scraper
# ----------------------------------------------------------
def scrape_audit_scotland(since_date=None, until_date=None, output_path=None, append=False):
    all_articles = []
    seen = set()
    url = START_URL
    page = 1

    print("Starting Audit Scotland scrape...")
...
if __name__ == "__main__":
    scrape_audit_scotland(output_path=_DEFAULT_OUTPUT)
