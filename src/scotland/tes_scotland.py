...
# Config
# ----------------------------------------------------------
BASE = "https://www.tes.com"
START_URL = "https://www.tes.com/magazine/hub/scotland"

_DEFAULT_OUTPUT = Path(__file__).resolve().parents[2] / "data" / "training" / "scotland" / "tes_scotland.csv"
...
def extract_links(html):
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"].strip()
        if "/magazine/" in href and "/hub/" not in href:
...
# ----------------------------------------------------------
# Scrape a single article (open articles only — skip paywalled)
# ----------------------------------------------------------
def scrape_article(url, since_date=None, until_date=None):
    ...
    if date_tag:
        ...
        for fmt in ["%Y-%m-%dT%H:%M:%S", "%Y-%m-%d", "%d %B %Y"]:
            try:
                pub_date = datetime.strptime(date_text[:19] if "T" in fmt else date_text, fmt).date()
                break
            except ValueError:
                continue
    ...
    content = soup.find("article") or soup.find("div", class_=lambda c: c and "article-body" in c.lower() if c else False)
    if content:
    ...
    if not text:
        print(f"    No text content — skipping: {url}")
        return None
...
# Main scraper
# ----------------------------------------------------------
def scrape_tes_scotland(since_date=None, until_date=None, output_path=None, append=False):
    all_articles = []
    seen = set()
    url = START_URL
    page = 1

    print("Starting TES Scotland scrape...")
...
if __name__ == "__main__":
    scrape_tes_scotland(output_path=_DEFAULT_OUTPUT)
